...
/*  Subscribe to DOSD TLM. used as test
    Result = CFE_SB_Subscribe (DOSD_HK_TLM_MID,
                               CS_AppData.CmdPipe);
    */
    if (Result != CFE_SUCCESS)
    {
        CFE_EVS_SendEvent (CS_INIT_SB_SUBSCRIBE_HK_ERR_EID,
                           CFE_EVS_ERROR,
                           "Software Bus subscribe to DOSD returned: 0x%08X",(unsigned int)Result);
        return Result;
    }
...
int32 CS_AppPipe (CFE_SB_MsgPtr_t MessagePtr)
{
    ...
    switch (MessageID)
    {
          /*check if subscription successful and receiving from DOSD
	 case DOSD_HK_TLM_MID:
		CFE_EVS_SendEvent (CS_MID_ERR_EID, CFE_EVS_ERROR,
                               "DOSD HK RECEIVED: 0x%04X",
                               MessageID);
	break; */
